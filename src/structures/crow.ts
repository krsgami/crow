...
import { formatError } from "../functions/error.js";
...
export class CROW extends Client {
  ...
  declare public logger: typeof Logger;
  ...
  async loadCommands(commandPath: string): Promise<void> {
    try {
      ...
      for (const category of categories) {
        ...
        for (const file of commandFiles) {
          ...
          if (typeof CommandClass !== "function") {
            this.logger.warn(
              `Comando inválido (não é uma classe) em ${filePath}`,
            );
            continue;
          }
          ...
          if (command) {
            ...
            this.logger.module(
              `Loaded ${chalk.cyanBright(typeLabel)}: ${chalk.yellowBright(name)}`,
            );
          } else {
      ...
      this.logger.info(
        chalk.cyanBright(`${commandCount} comando(s) carregado(s)`),
      );
    } catch (error) {
  ...
  async loadEvents(eventPath: string): Promise<void> {
    try {
      ...
      for (const category of categories) {
        ...
        for (const file of eventFiles) {
          ...
          if (event && event.name) {
            ...
            eventCount++;
            this.logger.module(
              `Evento carregado: ${chalk.yellowBright(
                event.name,
              )} - ${chalk.magentaBright(
                file,
              )} da categoria: ${chalk.blueBright(category)}`,
            );
          } else {
            this.logger.warn(
              `Falha ao carregar evento em ${filePath} - Propriedade ${chalk.yellowBright(
                "'name'",
              )} está faltando`,
            );
          }
      ...
      this.logger.info(
        chalk.greenBright(`${eventCount} evento(s) carregado(s)`),
      );
    } catch (error) {
  ...
  async registerSlashCommands(): Promise<void> {
    try {
      ...
      this.logger.success(
        `Comando(s) de barra (/) ${chalk.cyanBright(
          (data as any[]).length + " registrado(s) com sucesso!",
        )}`,
      );
    } catch (error) {
      this.logger.error(
        `Falha ao registrar comandos de barra (/): ${formatError(error)}`,
      );
    }
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.login(process.env.CROW_TOKEN!).then(() =>
        this.logger.success(
          `${chalk.magenta("crow")} iniciou com sucesso!`,
        ),
      );
    } catch (error) {
